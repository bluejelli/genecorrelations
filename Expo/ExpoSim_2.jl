...
begin
    ...
    cov_out_2 = timeseries_point_meancov(exp_sol_2, ts, ts)

    ts2= t0:1.0:tf
    all_As_2 = [componentwise_vectors_timepoint(exp_sol_2, i)[1] for i in collect(ts2)]
    all_Bs_2 = [componentwise_vectors_timepoint(exp_sol_2, i)[2] for i in collect(ts2)]
end

begin
    jldsave("ExpoSim_2_raw_full.jld2"; ts, cov_out_2, ts2, all_As_2, all_Bs_2)
end
