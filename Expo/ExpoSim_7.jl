...
begin
    ...
    cov_out_7 = timeseries_point_meancov(exp_sol_7, ts, ts)

    ts2= t0:1.0:tf
    all_As_7 = [componentwise_vectors_timepoint(exp_sol_7, i)[1] for i in collect(ts2)]
    all_Bs_7 = [componentwise_vectors_timepoint(exp_sol_7, i)[2] for i in collect(ts2)]
end

begin
    jldsave("ExpoSim_7_raw_full.jld2"; ts, cov_out_7, ts2, all_As_7, all_Bs_7)
end
