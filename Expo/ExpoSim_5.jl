...
begin
    ...
    cov_out_5 = timeseries_point_meancov(exp_sol_5, ts, ts)

    ts2= t0:1.0:tf
    all_As_5 = [componentwise_vectors_timepoint(exp_sol_5, i)[1] for i in collect(ts2)]
    all_Bs_5 = [componentwise_vectors_timepoint(exp_sol_5, i)[2] for i in collect(ts2)]
end

begin
    jldsave("ExpoSim_5_raw_full.jld2"; ts, cov_out_5, ts2, all_As_5, all_Bs_5)
end
