...
begin
    ...
    cov_out_8 = timeseries_point_meancov(exp_sol_8, ts, ts)

    ts2= t0:1.0:tf
    all_As_8 = [componentwise_vectors_timepoint(exp_sol_8, i)[1] for i in collect(ts2)]
    all_Bs_8 = [componentwise_vectors_timepoint(exp_sol_8, i)[2] for i in collect(ts2)]
end

begin
    jldsave("ExpoSim_8_raw_full.jld2"; ts, cov_out_8, ts2, all_As_8, all_Bs_8)
end
