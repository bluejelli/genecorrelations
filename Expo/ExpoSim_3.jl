...
begin
    ...
    cov_out_3 = timeseries_point_meancov(exp_sol_3, ts, ts)

    ts2= t0:1.0:tf
    all_As_3 = [componentwise_vectors_timepoint(exp_sol_3, i)[1] for i in collect(ts2)]
    all_Bs_3 = [componentwise_vectors_timepoint(exp_sol_3, i)[2] for i in collect(ts2)]
end

begin
    jldsave("ExpoSim_3_raw_full.jld2"; ts, cov_out_3, ts2, all_As_3, all_Bs_3)
end
